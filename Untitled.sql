create database fsp_project;
use fsp_project;

CREATE OR REPLACE TABLE district_prices_monthly (
    year_month STRING,district STRING,okrug STRING,secondary_price_per_sqm INTEGER,
    newbuild_price_per_sqm INTEGER,rental_price_per_sqm_monthly INTEGER,   
    secondary_mom_change_pct FLOAT,newbuild_mom_change_pct FLOAT,    
    n_listings_secondary INTEGER,n_listings_newbuild INTEGER,n_listings_rental INTEGER,    
    cbr_key_rate_pct FLOAT,avg_mortgage_rate_pct FLOAT
);

CREATE OR REPLACE TABLE rentals (
    id STRING,date_posted DATE,  district STRING,okrug STRING,  lat FLOAT,lon FLOAT,    
    total_area FLOAT, rooms INTEGER,floor INTEGER,total_floors INTEGER,    
    building_year INTEGER, building_type STRING,renovation STRING, furnished BOOLEAN,
    pets_allowed BOOLEAN, deposit_months INTEGER, metro_station STRING,
    metro_line STRING,metro_distance_min INTEGER, to_center_km FLOAT,    
    monthly_rent_rub INTEGER,rent_per_sqm INTEGER
);

CREATE OR REPLACE TABLE secondary_market (
    id STRING,date_posted DATE, district STRING,okrug STRING, lat FLOAT,lon FLOAT,    
    total_area FLOAT,living_area FLOAT,kitchen_area FLOAT, rooms INTEGER,
    floor INTEGER,total_floors INTEGER, building_year INTEGER, building_type STRING,    
    ceiling_height FLOAT, has_balcony BOOLEAN, renovation STRING, metro_station STRING,
    metro_line STRING,metro_distance_min INTEGER,metro_distance_type STRING,    
    to_center_km FLOAT, price_rub INTEGER,price_per_sqm INTEGER,    
    mortgage_rate_at_listing FLOAT,  seller_type STRING
);
...
copy into district_prices_monthly
from @tmslstage/district_prices_monthly.csv
file_format = (type =csv skip_header = 1);

COPY INTO rentals
FROM @tmslstage/rentals.csv
FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1);

COPY INTO secondary_market
FROM @tmslstage/secondary_market.csv
FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1);
